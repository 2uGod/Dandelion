...
const normalizeCrop = (c) => {
  const name =
    c?.name ?? c?.data?.name ?? c?.crop?.name ?? c?.title ?? c?.label ?? "";
  const id = c?.id ?? c?.data?.id ?? c?.crop?.id ?? c?.uuid ?? c?._id ?? null;
...
const PlantSidebar = ({ selectedPlant, setSelectedPlant, onCropAdded }) => {
  ...
  const handleAdded = (newCropRaw) => {
    ...
    if (added.id || added.name)
      onCropAdded?.({ id: added.id, name: added.name });
    setShowPopup(false);
  };
