...
export default function ProfileSettings() {
    ...
    try {
      ...
      if (avatarDirty) {
        if (avatarFile instanceof File) {
          ...
          newAvatarUrl = normalized
            ? withCacheBust(normalized)
            : profile.avatarUrl;
        } else if (
          profile.avatarPreview &&
          profile.avatarPreview.startsWith("data:")
        ) {
  ...
  const displayAvatar =
    profile.avatarPreview || profile.avatarUrl || withCacheBust(DEFAULT_AVATAR);

  const anythingDirty = avatarDirty || nicknameDirty || interestsDirty;

  const uploaderBoxRef = useRef(null);
  const openFileDialog = () => {
    const input = uploaderBoxRef.current?.querySelector('input[type="file"]');
    ...
    <section className="settings-wrap">
      ...
      <div className="settings-grid">
        <div className="settings-col-left">
          <div className="avatar-card" ref={uploaderBoxRef}>
            <AvatarUploader
              id="avatar"
              label="프로필 이미지"
              value={displayAvatar}
        ...
        <div className="settings-col-right">
          <div className="settings-row">
            <label className="settings-label" htmlFor="farmunityId">
              Farmunity ID
            </label>
            <input
              id="farmunityId"
