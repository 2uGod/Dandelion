...
const MainCalendar = ({ plant, tasks = [], onGoPlan, onEventClick }) => {
  ...
  const dayTasks = useMemo(() => {
    if (isCommonView) return [];
    return tasks.filter(
      (t) => t.date === selectedKey && (t.plant || "공통") === plant
    );
      ...
      <div className="cal-dots-wrap">
        {first3.map((t, i) => (
          <span
            ...
            className="cal-dot"
            style={{
              backgroundColor: t.color || plantColor(t.plant || "공통"),
            }}
      ...
      <aside className="schedule-pane">
        <div className="schedule-card">
          ...
          <div className="schedule-actions">
            ...
            <button
              className="btn-ghost"
              onClick={() => alert("메모 기능은 추후 연결 예정입니다.")}
            >
              메모
            </button>
          </div>
